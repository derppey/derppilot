/**
 * Copyright (c) 2021-, Haibin Wen, chubbs, and a number of other contributors.
 *
 * This file is part of chubbs and is licensed under the MIT License.
 * See the LICENSE.md file in the root directory for more details.
 */

#pragma once

#include <map>
#include <string>

#include "selfdrive/ui/chubbs/ui.h"
#include "selfdrive/ui/chubbs/qt/offroad/settings/chubbs/mads_settings.h"
#include "selfdrive/ui/qt/util.h"
#include "selfdrive/ui/chubbs/qt/offroad/settings/settings.h"
#include "selfdrive/ui/chubbs/qt/widgets/scrollview.h"

class ChubbsPanel : public QFrame {
  Q_OBJECT

public:
  explicit ChubbsPanel(SettingsWindowSP *parent = nullptr);
  void showEvent(QShowEvent *event) override;
  void hideEvent(QHideEvent* event) override;

public slots:
  void updateToggles(bool _offroad);

private:
  QStackedLayout* main_layout = nullptr;
  QWidget* chubbsScreen = nullptr;
  ScrollViewSP *chubbsScroller = nullptr;
  std::vector<ParamControl *> toggleOffroadOnly;
  bool offroad;

  ParamControl *madsToggle;
  PushButtonSP *madsSettingsButton;
  MadsSettings *madsWidget = nullptr;
};
